 ...
 *   it can also manage the removal of associated event listeners, leveraging integration with an SimplyBuilderListener for comprehensive event management.
 *
 ...
 * behind a consistent and intuitive API. It's designed to be used in conjunction with other modules like SimplyBuilderListener to provide a complete
 ...
 * If mode is set to 1 and an SimplyBuilderListener is provided, this function will also remove all associated event listeners.
 *
 ...
 * @param {Object} [data.SimplyBuilderListener] - An optional module for managing event listeners on the element.
 */
const removeElementFromStore = (data = {}) => {
    const {key, mode = 1, SimplyBuilderListener = {}} = data;
    try {
        const modeTypes = {
            1: (key) => {
                if (typeof SimplyBuilderListener.removeAllEventsFromStore === "function") SimplyBuilderListener.removeAllEventsFromStore(ElementRefStore[key]);
